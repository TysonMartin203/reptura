...
const MUSCLE_GROUPS = {
  // Chest
  'Bench Press': ['Chest', true], 'Incline Bench Press': ['Chest', true], 'Decline Bench Press': ['Chest', true],
...
function reorderExercisesByMuscleGroup(exercises) {
  ...
  const withMeta = lifting.map(e => {
    const [muscle, compound] = MUSCLE_GROUPS[e.exerciseName] || ['Other', true];
    return { ex: e, muscle, compound };
  });

  // Group by muscle, compounds first within each group (stable order preserved otherwise).
  const byMuscle = {};
...
module.exports = { MUSCLE_GROUPS, reorderExercisesByMuscleGroup };
