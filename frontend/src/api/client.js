...
export const api = {
  ...
  getKrogerConnectUrl: ()            => request('GET',    '/api/kroger/connect'),
  getKrogerStatus:     ()            => request('GET',    '/api/kroger/status'),
  disconnectKroger:    ()            => request('DELETE', '/api/kroger/disconnect'),
  searchKrogerStores:  (zip)         => request('GET',    `/api/kroger/stores?zip=${encodeURIComponent(zip)}`),
  setKrogerLocation:   (locationId, name) => request('POST', '/api/kroger/location', { locationId, name }),
  addToKrogerCart:     (items)       => request('POST',   '/api/kroger/add-to-cart', { items }),
  getFamilyMembers:   ()              => request('GET',    '/api/family-members'),
  getRaceOptions:   ()      => request('GET',    '/api/race-plans/options'),
  getRacePlans:     ()      => request('GET',    '/api/race-plans'),
  getRacePlan:      (id)    => request('GET',    `/api/race-plans/${id}`),
  createRacePlan:   (body)  => request('POST',   '/api/race-plans', body),
  deleteRacePlan:   (id)    => request('DELETE', `/api/race-plans/${id}`),
  addFamilyMember:    (member)        => request('POST',   '/api/family-members', member),
  updateFamilyMember: (id, member)    => request('PUT',    `/api/family-members/${id}`, member),
  deleteFamilyMember: (id)            => request('DELETE', `/api/family-members/${id}`),
